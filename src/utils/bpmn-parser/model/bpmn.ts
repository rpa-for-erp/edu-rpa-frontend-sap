...
export class BpmnStartEvent extends BpmnNode {
  ...
  public constructor(element: BpmnElement) {
    ...
    this.outgoing = (element.elements || [])
      .filter((e) => e.name === "bpmn:outgoing")
...
export class BpmnEndEvent extends BpmnNode {
  ...
  public constructor(element: BpmnElement) {
    super();
    this.id = element.attributes.id;
    this.incomming = (element.elements || [])
      .filter((e) => e.name === "bpmn:incoming")
      .map((e) => e.elements?.[0]?.text || "");
  }
...
export class BpmnParallelGateway extends BpmnNode {
  id: string = "";
  name: string = "";
  incomming: string[] = [];
  outgoing: string[] = [];
...
export class BpmnInclusiveGateway extends BpmnNode {
  id: string = "";
  name: string = "";
  incomming: string[] = [];
  outgoing: string[] = [];
...
export class BpmnProcess extends BpmnNode {
  ...
  public check() {
    let startEventId = Object.keys(this.elements).find(
      (key) => this.elements[key] instanceof BpmnStartEvent
    );
    if (!startEventId)
      throw new BpmnParseError("Not Found Start Event", this.id);

    let endEventId = Object.keys(this.elements).find(
      (key) => this.elements[key] instanceof BpmnEndEvent
    );
    if (!endEventId) throw new BpmnParseError("Not Found End Event", this.id);
  }
  public toString(indent: number): string {
    return "Process";
  }
