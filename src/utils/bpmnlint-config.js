...
function checkDiscouragedNodeType$2(type, ruleName) {
  ...
  return function() {
    ...
    return annotateRule$n(ruleName, {
      check
    });
 ...
 */
function annotateRule$n(ruleName, options) {
...
helper.annotateRule = annotateRule$n;
...
var adHocSubProcess = function() {
  ...
  function check(node, reporter) {
    ...
    if (!is$j(node, 'bpmn:AdHocSubProcess')) {
      return;
    }
...
var conditionalFlows = function() {
  ...
  return annotateRule$l('conditional-flows', {
    check
  });
...
var endEventRequired = function() {
  ...
  function check(node, reporter) {
    ...
    if (!isAny$9(node, [
      'bpmn:Process',
      'bpmn:SubProcess'
    ]) || is$i(node, 'bpmn:AdHocSubProcess')) {
      return;
    }
  ...
  return annotateRule$k('end-event-required', {
    check
  });
};
 ...
 * @type { import('../lib/types.js').RuleFactory }
 */
var eventBasedGateway = function() {

  function check(node, reporter) {

    if (!is$h(node, 'bpmn:EventBasedGateway')) {
      return;
    }
...
var noDisconnected = function() {
  ...
  function check(node, reporter) {
    ...
    if (is$b(node.$parent, 'bpmn:AdHocSubProcess')) {
      return;
    }
...
var noGatewayJoinFork = function() {
  ...
  return annotateRule$a('no-gateway-join-fork', {
    check
  });
...
var noImplicitEnd = function() {
  ...
  function isImplicitEnd(node) {
    ...
    if (is$8(node.$parent, 'bpmn:AdHocSubProcess')) {
      return false;
    }
    ...
    if (is$8(node, 'bpmn:Activity') && isForCompensation(node)) {
      return false;
    }
...
var noImplicitStart = function() {
  ...
  function isImplicitStart(node) {
    ...
    if (is$7(node, 'bpmn:Activity') && isForCompensation(node)) {
      return false;
    }

    if (is$7(node.$parent, 'bpmn:AdHocSubProcess')) {
      return false;
    }
...
cache['bpmnlint/event-based-gateway'] = eventBasedGateway;
