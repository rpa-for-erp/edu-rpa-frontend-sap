...
import RobotLog from '@/pages/robot/components/Log/RobotLog';
import { LOG_ROBOT } from '@/constants/robot';
import LoadingIndicator from '@/components/LoadingIndicator/LoadingIndicator';
import ConnectionDetail from '@/pages/robot/components/ConnectionDetail/ConnectionDetail';
import LogDetail from '@/pages/robot/components/LogDetail/LogDetail';
import RobotDashboard from '@/pages/robot/components/Dashboard/RobotDashboard';
...
const WorkspaceRobotDetail = () => {
  ...
  const {
    ...
    queryKey: [QUERY_KEY.ROBOT_STATUS, workspaceId, processID],
    queryFn: () => workspaceApi.getWorkspaceRobotById(workspaceId as string, robotID),
    enabled: !!workspaceId && !!robotID,
  });

  const handleRunRobot = useMutation({
    mutationFn: async (payload: RobotRunningDto) => {
      // TODO: Implement workspace robot run API
  ...
  const handleStopRobot = useMutation({
    mutationFn: async (payload: RobotRunningDto) => {
      // TODO: Implement workspace robot stop API
      return Promise.resolve();
    },
    onSuccess: () => {
      toast({
    ...
    <Box className="bg-white h-[100vh]">
      <Box className="flex justify-between items-center w-90 m-auto">
        <IconButton
          ...
          isRound={true}
          size="lg"
          onClick={() => router.push(`/workspace/${workspaceId}/robot`)}
...
export default WorkspaceRobotDetail;
